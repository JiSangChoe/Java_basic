...
// JVM의 메모리 구조
// 메서드 영역: 모든 쓰레드가 공유하며 클래스의 구조 정보와 static으로 선언된 변수 혹은 메스드가 저장되는 공간
// 힙 영역: 모든 쓰레드가 공유하며 클래스로 생성된 인스턴스와 배열이 저장되는 공간 (가비지 컬렉션의 주요 대상)
// 스텍 영역: 각 쓰레드 별로 존재하는 공간, 로컬 변수나 메서드 호출 데이터가 저장되는 공간(각 메서드가 종료되면 제거됨)
...
public class A_JVM {
	...
	// 메인 메서드 실행되면 실행시 필요한 데이터는 '스텍 영역'에 저장된다.
	public static void main(String[] args) {
		
		// 지역 변수의 데이터는 '스택 영역'에 저장된다.
		int number = 0;
		
		// 지역 참조 변수는 '스텍 영역'에 저장된다.
